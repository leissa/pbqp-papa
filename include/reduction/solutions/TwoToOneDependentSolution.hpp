...
public:
	TwotoOneDependentSolution(PBQPNode<T>* toSolve, PBQPNode<T>* dependencyNode1, PBQPNode<T>* dependencyNode2) :
			toSolve(toSolve), dependencyNode1(dependencyNode1), dependencyNode2(dependencyNode2),
			firstDependencyVector(dependencyNode1->getVector()), secondDependencyVector(dependencyNode2->getVector()) {
	...
	void setSolutionSelection(unsigned short dependency1Selection, unsigned short dependency2Selection,
		...
		selection.at(resolveIndex(dependency1Selection, dependency2Selection)) = solutionSelection;
	}
	...
	void revertChange(PBQPGraph<T>* graph) override {
		...
		graph->addEdge(firstEdge.getSource(), firstEdge.getTarget(), firstEdge.getMatrix());
		graph->addEdge(secondEdge.getSource(), secondEdge.getTarget(), secondEdge.getMatrix());
